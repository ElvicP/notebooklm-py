...
class TestLanguageSetCommand:
    ...
    def test_language_set(self, runner, mock_auth_for_vcr, tmp_path, monkeypatch):
        ...
        Redirects ``NOTEBOOKLM_HOME`` to ``tmp_path/.notebooklm`` so the test
        never touches the real user's config. We must NOT use ``$HOME``: on
        Windows ``Path.home()`` resolves via ``USERPROFILE`` and ignores
        ``$HOME``, so an ``$HOME`` override leaks the write to the real
        profile (the Windows-only failure this fixes). ``get_home_dir``
        honors ``NOTEBOOKLM_HOME`` first on every platform, so the
