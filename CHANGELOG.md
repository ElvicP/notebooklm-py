...
- **Proactive SIDTS rotation poke** - Every `fetch_tokens` call now makes a best-effort GET to `https://accounts.google.com/CheckCookie` before hitting `notebooklm.google.com`. Google only emits a rotated `__Secure-1PSIDTS` (the freshness partner of `__Secure-1PSID`) when the client touches the identity surface; pure RPC traffic against `notebooklm.google.com` never triggers rotation, so a keepalive that hits NotebookLM alone silently stales out and dies after ~10-30 minutes. The rotated `Set-Cookie` lands in the live httpx jar and is persisted by `ClientCore.close` (#312). Failures are logged at DEBUG and never abort token fetch. Disable with `NOTEBOOKLM_DISABLE_KEEPALIVE_POKE=1` (e.g. networks that block `accounts.google.com`).
